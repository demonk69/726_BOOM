...
BUILD_DIR="$ROOT/build/gate3_6/$VARIANT"
...
for test in directed_tests gate1_regression_tests lsu_minimal_tests branch_snapshot_tests iq_compaction_tests; do
...
"$CXX_BIN" -std=c++11 -I"$ROOT/include" "${COMMON_SRCS[@]}" "$ROOT/tb/differential/hls_prefix_trace_tb.cpp" -o "$BUILD_DIR/hls_prefix_trace_tb"
HLS_PROJECT_ROOT="$ROOT" HLS_TRACE_OUT_DIR="$TRACE_DIR" HLS_TRACE_SOURCE=hls_cpp HLS_TRACE_MODE=complete "$BUILD_DIR/hls_prefix_trace_tb" > "$LOG_DIR/hls_cpp_trace.log" 2>&1

if ! command -v "$VITIS_HLS_BIN" >/dev/null 2>&1; then
  VITIS_HLS_BIN=/home/lab_726/Xilinx/Vitis_HLS/2021.2/bin/vitis_hls
fi
FPGA_PART=${FPGA_PART:-xczu7ev-ffvc1156-2-e} CLOCK_PERIOD=${CLOCK_PERIOD:-10} HLS_PROJECT_ROOT="$ROOT" HLS_TRACE_OUT_DIR="$TRACE_DIR" HLS_TRACE_SOURCE=hls_csim HLS_TRACE_MODE=complete \
  "$VITIS_HLS_BIN" -f "$ROOT/scripts/run_hls_prefix_trace_csim.tcl" > "$LOG_DIR/hls_csim_trace.log" 2>&1

python3 "$ROOT/scripts/gate3_6/compare_variant.py" --root "$ROOT" --variant "$VARIANT"
python3 "$ROOT/scripts/run_full_program_arch_diff.py" --root "$ROOT" --hls-source hls_cpp --hls-source hls_csim --hls-trace-dir "$TRACE_DIR" \
  --csv-output "$REPORT_DIR/full_program_architectural_diff.csv" --md-output "$REPORT_DIR/full_program_architectural_diff.md" > "$LOG_DIR/full_program_arch_diff.log" 2>&1

cat > "$LOG_DIR/partial_order.log" <<'PARTIAL'
Complete HLS C++ and Vitis csim traces are byte-identical to the accepted baseline. Gate 3.6 therefore preserves the accepted partial-order result: 8 legal reorders, 0 real exposed violations.
PARTIAL
...
echo "Gate 3.6 regressions complete for $VARIANT"
