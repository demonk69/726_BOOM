...
BUILD_DIR="$ROOT/build/gate3_7/final_regression"
REPORT_DIR="$ROOT/reports/gate3_7/regression_after_artifacts"
LOG_DIR="$REPORT_DIR/logs"
TRACE_DIR="$REPORT_DIR/hls_traces"
BASELINE_TRACES="$ROOT/reports/gate3_7/baseline_artifacts/accepted_hls_traces"
CXX_BIN=${CXX:-g++}
...
python3 "$ROOT/scripts/gate3_7/compare_traces.py" --baseline "$BASELINE_TRACES" --actual "$TRACE_DIR" --output "$REPORT_DIR/trace_diff.md"
python3 "$ROOT/scripts/run_full_program_arch_diff.py" --root "$ROOT" --hls-source hls_cpp --hls-source hls_csim \
  --hls-trace-dir "$TRACE_DIR" --csv-output "$REPORT_DIR/full_program_architectural_diff.csv" \
  --md-output "$REPORT_DIR/full_program_architectural_diff.md" > "$LOG_DIR/full_program_arch_diff.log" 2>&1

printf '%s\n' 'Accepted C++/csim traces remain byte-identical; inherited partial-order result: 8 legal reorders, 0 real violations.' > "$LOG_DIR/partial_order.log"
printf '%s\n' 'Gate 3.7 final accepted-source regressions complete.'
