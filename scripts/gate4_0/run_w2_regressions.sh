...
BUILD_DIR="$ROOT/build/gate4_0/w2_regression"
REPORT_DIR="$ROOT/reports/gate4_0/w2/regression"
...
"$CXX_BIN" -std=c++11 -I"$ROOT/include" "$ROOT/src/issue.cpp" "$ROOT/src/reset.cpp" \
  "$ROOT/tb/differential/w2_dual_grant_tests.cpp" -o "$BUILD_DIR/w2_dual_grant_tests"
"$CXX_BIN" -std=c++11 -I"$ROOT/include" "$ROOT/src/issue.cpp" \
  "$ROOT/tb/differential/w2_dual_grant_random_tests.cpp" -o "$BUILD_DIR/w2_dual_grant_random_tests"
...
for seed in default 0x13579bdf 0x2468ace0 0x10203040 0x55667788 0xdeadbeef 0x0badc0de \
  0xc001d00d 0x31415926 0x27182818 0xabcdef01 0x12345678 0x87654321 0xfeedface \
  0x600dcafe 0x5eed1234 0x01020304 0x89abcdef 0xf00df00d 0x55aa55aa 0xaa55aa55; do
...
python3 "$ROOT/scripts/gate3_7/compare_traces.py" --baseline "$BASELINE_TRACES" \
  --actual "$TRACE_DIR" --output "$REPORT_DIR/trace_diff.md" --title "Gate 4.0 W2 Trace Diff"
...
python3 "$ROOT/scripts/build_dynamic_uop_map.py" --root "$ROOT" \
  --out-dir "$ROOT/reports/gate4_0/w2/partial_order" > "$LOG_DIR/partial_order.log" 2>&1

printf '%s\n' 'Gate 4.0 W2 regressions complete.'
