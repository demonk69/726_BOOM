...
BUILD_DIR="$ROOT/build/gate4_0/w1_regression"
REPORT_DIR="$ROOT/reports/gate4_0/w1/regression"
...
for test in directed_tests gate1_regression_tests lsu_minimal_tests branch_snapshot_tests \
  iq_compaction_tests gate4_w1_lane_interface_tests reset_architecture_tests; do
...
python3 "$ROOT/scripts/gate3_7/compare_traces.py" --baseline "$BASELINE_TRACES" \
  --actual "$TRACE_DIR" --output "$REPORT_DIR/trace_diff.md" --title "Gate 4.0 W1 Trace Diff"
...
printf '%s\n' 'Gate 4.0 W1 regressions complete.'
