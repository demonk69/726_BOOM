`timescale 1ns/1ps
module pf3_product_rtl_harness (
    ...
    output wire observed_imem_transfer, output wire [4:0] ftq_head,
    output wire [7:0] ftq_tail, output wire [7:0] ftq_count,
    output wire [31:0] ftq_next_generation, output wire ftq_retire_pending,
    output wire ftq_redirect_pending, output wire exception_valid
);
    ...
    wire [31:0] imem_transfers, imem_responses, dmem_transfers, dmem_responses;
    wire [31:0] dmem_loads, dmem_stores, trace_transfers;
    wire imem_pending, dmem_pending, branch_commit_visible;
    ...
    assign ftq_head = dut.state_ftq_head_s;
    assign ftq_tail = dut.state_ftq_tail_s;
    assign ftq_count = dut.state_ftq_count_s;
    assign ftq_next_generation = dut.state_ftq_next_generation_s;
    assign ftq_retire_pending = dut.state_ftq_retire_pending_valid;
    assign ftq_redirect_pending = dut.state_ftq_redirect_pending_valid;
